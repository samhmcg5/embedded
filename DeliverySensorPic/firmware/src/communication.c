...
struct periphQueueData parseJSON (unsigned char rec[UART_RX_QUEUE_SIZE]) 
{
    struct periphQueueData out;
    ...
    if(r == 5) // ACTION
    {
        out.type = COMMTASK_TO_PERIPHERAL_MSG;
        out.magnet  = getIntFromKey(t[4]); //o for off 1 for on
    }
    else // ERROR
    {
    ...
    switch (communicationData.state) 
    {
        ...
        case COMM_AWAIT_RESPONSE:
        {
            ...
            if(xQueueReceive(comm_incoming_q, &rec, portMAX_DELAY))
            {
                ...
                struct  periphQueueData out = parseJSON(rec);
                sendMsgToPeriphQ(out);
            }
            break;
        }
        default:
