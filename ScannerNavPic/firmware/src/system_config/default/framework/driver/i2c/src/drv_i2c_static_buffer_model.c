...
void _DRV_I2C_Advance_Queue_1(void)
{
...
void DRV_I2C1_Initialize(void)
{
    ...
    i2c1Obj.i2cMode         = DRV_I2C_MODE_MASTER;
...
void DRV_I2C1_Tasks(void)
{
    ...
    switch (i2c1State)
    {
        case DRV_I2C_TASK_SEND_DEVICE_ADDRESS:
        {
            if (i2c1Obj.i2cMode == DRV_I2C_MODE_MASTER)
            {
                ...
                if (PLIB_I2C_StartWasDetected(I2C_ID_2))
                {
                    if ( (lBufferObj1->i2coperation == DRV_I2C_OP_WRITE) || (lBufferObj1->i2coperation == DRV_I2C_OP_WRITE_READ) )
                    {
                        ...
                        PLIB_I2C_TransmitterByteSend( I2C_ID_2, lBufferObj1->slaveaddresshighbyte );
                    }
                    else
                    {
                        lBufferObj1->bufferstatus    = DRV_I2C_BUFFER_SLAVE_READ_REQUESTED;
                        i2c1State = DRV_I2C_TASK_SET_RCEN_ONLY;
                        PLIB_I2C_TransmitterByteSend(I2C_ID_2, lBufferObj1->slaveaddresshighbyte | DRV_I2C_OP_READ );
                    }
                }
            }
            else if (i2c1Obj.i2cMode == DRV_I2C_MODE_SLAVE)
            {
                ...
                else if ( (PLIB_I2C_SlaveReadIsRequested(I2C_ID_2)) && (PLIB_I2C_SlaveAddressIsDetected(I2C_ID_2)) )
                {
                    ...
                    // transmit first byte
                    PLIB_I2C_TransmitterByteSend(I2C_ID_2, *lBufferObj1->txBuffer++);
                ...
                else if ( (!PLIB_I2C_SlaveReadIsRequested(I2C_ID_2)) && PLIB_I2C_SlaveDataIsDetected(I2C_ID_2))
                {
                    ...
                    {
                        *lBufferObj1->rxBuffer++ = PLIB_I2C_ReceivedByteGet(I2C_ID_2);
                        lBufferObj1->transfersize++;
        ...
        case DRV_I2C_SEND_DEVICE_ADDRESS_BYTE_2:
        {
            ...
            {
                PLIB_I2C_TransmitterByteSend(I2C_ID_2,lBufferObj1->slaveaddresslowbyte>>1) ;
                i2c1State = DRV_I2C_TASK_SET_RCEN_ONLY;
            }
            else
            {
...
// *****************************************************************************
// Section: Instance 1 Function to check status of a buffer
//          transfer operation
// *****************************************************************************
